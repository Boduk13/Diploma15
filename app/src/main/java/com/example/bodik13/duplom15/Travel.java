package com.example.bodik13.duplom15;

/**
 * Created by Bodik13 on 31.03.2015.
 */
public class Travel {
    public String TAG_ID;
    public String TAG_startPoint;
    public String TAG_finishPoint;
    public String TAG_intermedientePoints;
    public String TAG_price;
    public String TAG_seats;
    public String TAG_dataFirst;
    public String TAG_dataLast;
    public String TAG_timeFirst;
    public String TAG_timeLast;
    public String TAG_firstName;
    public String TAG_lastName;
    public String TAG_adress;
    public String TAG_phone;
    public String TAG_mail;
    public String TAG_age;
    public String TAG_drivingExp;
    public String TAG_avatar;
    public String TAG_carBrand;
    public String TAG_carModel;
    public String TAG_carType;
    public String TAG_carPhoto;
    public String TAG_carColor;
    public String TAG_carSizeBaggage;
    public String TAG_carNumberSeats;
    public String TAG_carAnimals;
    public String TAG_carSmoke;


    public Travel(String TAG_ID, String TAG_startPoint, String TAG_finishPoint, String TAG_intermedientePoints, String TAG_price, String TAG_seats, String TAG_dataFirst, String TAG_dataLast, String TAG_timeFirst, String TAG_timeLast, String TAG_firstName, String TAG_lastName, String TAG_adress, String TAG_phone, String TAG_mail, String TAG_age, String TAG_drivingExp, String TAG_avatar, String TAG_carBrand, String TAG_carModel, String TAG_carType, String TAG_carPhoto, String TAG_carColor, String TAG_carSizeBaggage, String TAG_carNumberSeats, String TAG_carAnimals, String TAG_carSmoke) {
        this.TAG_ID = TAG_ID;
        this.TAG_startPoint = TAG_startPoint;
        this.TAG_finishPoint = TAG_finishPoint;
        this.TAG_intermedientePoints = TAG_intermedientePoints;
        this.TAG_price = TAG_price;
        this.TAG_seats = TAG_seats;
        this.TAG_dataFirst = TAG_dataFirst;
        this.TAG_dataLast = TAG_dataLast;
        this.TAG_timeFirst = TAG_timeFirst;
        this.TAG_timeLast = TAG_timeLast;
        this.TAG_firstName = TAG_firstName;
        this.TAG_lastName = TAG_lastName;
        this.TAG_adress = TAG_adress;
        this.TAG_phone = TAG_phone;
        this.TAG_mail = TAG_mail;
        this.TAG_age = TAG_age;
        this.TAG_drivingExp = TAG_drivingExp;
        this.TAG_avatar = TAG_avatar;
        this.TAG_carBrand = TAG_carBrand;
        this.TAG_carModel = TAG_carModel;
        this.TAG_carType = TAG_carType;
        this.TAG_carPhoto = TAG_carPhoto;
        this.TAG_carColor = TAG_carColor;
        this.TAG_carSizeBaggage = TAG_carSizeBaggage;
        this.TAG_carNumberSeats = TAG_carNumberSeats;
        this.TAG_carAnimals = TAG_carAnimals;
        this.TAG_carSmoke = TAG_carSmoke;
    }
}

